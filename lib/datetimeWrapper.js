...
var datetimeWrapper = function(moment) {
  ...
  return {
     ...
     */
    translateMask: function(timestr, MASK1, MASK2) {
      var res = moment(timestr, MASK1).format(MASK2);
      if (res !== 'Invalid date') {
        return res;
      }
      else {
        var e = new Error('Input datetime ' + timestr + ' did not match input mask ' + MASK1);
        console.error(e);
        throw e;
      }
    },
