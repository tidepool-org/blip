  ...
  return {
    ...
    getTimezones: function() {
      ...
      for (var i = 0; i < tzNames.length; ++i) {
        ...
        if (big4[tzNames[i]] === true) {
          timezones.bigFour.push(timezone);
        }
        else if (tzNames[i].search('US/') !== -1) {
          timezones.unitedStates.push(timezone);
        }
        else if (tzNames[i].search('Canada') !== -1) {
          timezones.hoisted.push(timezone);
        }
        else if (tzNames[i].search('Europe') !== -1) {
          timezones.hoisted.push(timezone);
        }
        else if (tzNames[i].search('Australia') !== -1) {
          timezones.hoisted.push(timezone);
        }
        else if (newZealand[tzNames[i]] === true) {
          timezones.hoisted.push(timezone);
        }
        // reformat these a tiny bit so that 'New Zealand' is searchable
        else if (tzNames[i].search('NZ') !== -1) {
