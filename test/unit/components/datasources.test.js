...
describe('DataSources', () => {
  ...
  describe('error message', () => {
    it('when unauthorized', () => {
      expect(
        wrapper.childAt(0).instance().calculateErrorMessage({ code: 'unauthenticated' })
      ).to.equal('Login expired - try signing out & in again');
    });

    it('when anything else', () => {
      expect(
        wrapper.childAt(0).instance().calculateErrorMessage('stuff')
  ...
  describe('state message', () => {
    it('when connected', () => {
      expect(
        wrapper.childAt(0).instance().calculateMessage(dataSources[0], 'connected')
      ).to.contain('Last data ');
    });

    it('when disconnected', () => {
      expect(
        wrapper.childAt(0).instance().calculateMessage(dataSources[0], 'disconnected')
      ).to.equal('No data available - click Connect to enable');
    });

    it('when anything else', () => {
      expect(
        wrapper.childAt(0).instance().calculateMessage(dataSources[0], 'hmmm')
      ).to.equal('An unknown error occurred');
    });
  });
  ...
  describe('calculating state', () => {
    it('when connected', () => {
      expect(
        wrapper.childAt(0).instance().calculateState({state: 'connected'})
      ).to.equal('connected');
    });

    it('when error', () => {
      expect(
        wrapper.childAt(0).instance().calculateState({state: 'error'})
      ).to.equal('error');
    });

    it('when disconnected', () => {
      expect(
        wrapper.childAt(0).instance().calculateState({state: 'disconnected'})
      ).to.equal('disconnected');
    });

    it('when other', () => {
      expect(
        wrapper.childAt(0).instance().calculateState({state: 'other'})
      ).to.equal('error');
    });

    it('when not set', () => {
      expect(
        wrapper.childAt(0).instance().calculateState()
      ).to.equal('disconnected');
    });
  });
  ...
  describe('calculate popup id', () => {
    it('appends given id to org.tidepool.web.', () => {
      expect(
        wrapper.childAt(0).instance().calculatePopupId({id: 'mine'})
