...
describe('clinics', () => {
  ...
  describe('acceptPatientInvitationSuccess', () => {
    it('should remove the patient invites from a clinic and increase the patientCount', () => {
      let initialStateForTest = {
        clinicId123: {
          patientCount: { demo: 1, plan: 2, total: 3 },
          patientInvites: {
      ...
      expect(state[clinic.id].patientCount).to.eql({ demo: 1, plan: 2, total: 3 });
    });
  });
    ...
    it('should add clinic patient and increase accounts by 1', () => {
      let initialStateForTest = {
        clinicId123: {
          ...
          patientCount: { demo: 0, plan: 0, total: 0 },
        },
      };
      let patient = {
        id: 'patient123',
      ...
      expect(state.clinicId123.patientCount).to.eql({ demo: 0, plan: 0, total: 0 });
    });
  });
    ...
    it('should remove patient and decrease counts by 1', () => {
      let initialStateForTest = {
        clinicId123: {
          ...
          patientCount: { demo: 0, plan: 1, total: 1 },
          clinicians: {
            clinicianId: {},
          },
        },
      ...
      expect(state.clinicId123.patientCount).to.eql({ demo: 0, plan: 1, total: 1 });
    });
  });
  ...
  describe('fetchClinicPatientCountSuccess', () => {
    it('should update `patientCount` in state', () => {
      ...
      let initialStateForTest = {
        [clinicId]: {
          id: clinicId,
          patientCount: { demo: 1, plan: 32, total: 33 },
        },
      };
      let action = actions.sync.fetchClinicPatientCountSuccess(clinicId, results);
      let state = reducer(initialStateForTest, action);
      expect(state[clinicId].patientCount).to.eql(results);
    });
  });
